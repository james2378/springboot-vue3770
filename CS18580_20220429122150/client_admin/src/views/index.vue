	...
	export default {
		...
		methods: {
			...
			async get_list_book_purchase() {
				...
				if (user_group!='管理员'){
					if (user_group=="教师用户"){
						data.buy_teachers = user_id;
					}
				}
				await this.$get(
					"~/api/book_purchase/get_list?groupby="+group_by_value,data,
					(json) => {
						if (json.result) {
			...
			async get_list_book_purchase_sub(v1,names_flag) {
				...
				await this.$get(
					"~/api/book_purchase/get_list?groupby=purchase_date",data,
					(json) => {
						if (json.result) {
							let list = json.result.list;
							let xAxis_list = [];
			...
			async get_list_book_purchase_sub_sub(v1,v2,names_flag,xAxis_flag) {
				...
				for (let i=0;i<this.line_obj_book_purchase.xAxis.length;i++){
					...
					for (let j=0;j<this.line_obj_book_purchase.names.length;j++){
						data[v2] = this.line_obj_book_purchase.xAxis[i];
						data[v1] = this.line_obj_book_purchase.names[j];
						await this.$get(
			...
			async get_list_book_receiving_information() {
				...
				await this.$get(
					"~/api/book_receiving_information/get_list?groupby="+group_by_value,data,
					(json) => {
						if (json.result) {
							let list = json.result.list;
							let name_list = [];
			...
			async get_list_book_receiving_information_sub(v1,names_flag) {
				...
				await this.$get(
					"~/api/book_receiving_information/get_list?groupby=purchase_quantity",data,
					(json) => {
						if (json.result) {
							let list = json.result.list;
							let xAxis_list = [];
							...
							this.get_list_book_receiving_information_sub_sub(v1,"purchase_quantity",names_flag,flag);
						}
				});
				let data = {};
				let flag = false;
				...
				await this.$get(
					"~/api/book_receiving_information/get_list?groupby=collection_date",data,
					(json) => {
						if (json.result) {
							let list = json.result.list;
							let xAxis_list = [];
